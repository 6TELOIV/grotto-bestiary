...
router.post("/login", async (req, res, next) => {
    try {
        ...
        const userResult = await db.collection("users").findOneAndUpdate(
            { id: user.id },
            { $set: { ...user, last_online: Date.now() } },
            { upsert: true, returnDocument: "after" });
        await db.collection("users").updateOne(
            { id: user.id, inventory: { $exists: false } },
            { $set: { inventory: [] } });
...
router.post("/trades", async (req, res, next) => {
    try {
        ...
        try {
            await sendTradeDMToUser(
                req.body.user.id,
                `${req.session.user.global_name ?? req.session.user.username} has sent you an offer!${req.body.tradeMessage ? `\n\nMessage:\n${req.body.tradeMessage}` : ""}`,
                `${getHostFromReq(req)}view-trade/${trade._id}`
            );
        } catch (e) {
            console.error(e);
        }
...
router.post("/trades", async (req, res, next) => {
    try {
        ...
        try {
            await sendTradeDMToUser(
                req.body.user.id,
                `${req.session.user.global_name ?? req.session.user.username} has ${req.body.action}ed your trade${req.body.action === "reject" ? "." : "!"}`,
                `${getHostFromReq(req)}view-trade/${trade._id}`
            );
        } catch (e) {
            console.error(e);
    ...
    for (const card of cards) {
        ...
        for (const rule of rules) {
            ...
            if (typeof rule.filter.name === "string") {
                ...
                if (!nameRegex.test(cardInfo.Name)) continue;
            }
            if (typeof rule.filter.type === "string") {
                const typeRegex = new RegExp(rule.filter.type?.replace(/\*/g, ".*"), "i");
                if (!typeRegex.test(cardInfo.Type)) continue;
            }

            // Rarity and holo must match exactly
            if (typeof rule.filter.rarity === "string" && rule.filter.rarity !== cardInfo.Rarity) continue;
            if (typeof rule.filter.holo === "boolean" && !rule.filter.holo !== !card.holo) continue;

            for (const tradeQtyCase of rule.tradeQty) {
                if (handleCase(tradeQtyCase, "TradeQty")) break;
            }
            for (const wishQtyCase of rule.wishQty) {
                if (handleCase(wishQtyCase, "WishQty")) break;
            }
