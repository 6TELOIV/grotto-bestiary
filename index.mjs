...
app.use("/card-details", (req, res, next) => {
    ...
    fs.readFile(
        ...
        (err, htmlData) => {
            ...
            htmlData = htmlData
                .replace(
                    "Violet's Grotto Bestiary",
                    `${req.query.cardName} - Grotto Bestiary`
                )
                .replace(
                    "Searchable Database for the hit 90's game, Grotto Beasts!",
                    ""
                )
                .replace(
                    "__META_OG_IMAGE__",
                    cardNameToCardImageURL(req.query.cardName)
                );
