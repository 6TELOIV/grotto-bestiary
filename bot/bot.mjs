                            ...
                            if (getImage[cardName]) {
                                ...
                                bot.createMessage(msg.channel.id, {
                                    ...
                                    embed: {
                                        title: card.Name,
                                        url: cardNameToCardDetailsURL(card.Name),
                                        description: `${card.Cost !== "-" ? `<:starbiticon:1128569464556900352>${card.Cost} ` : ""
                                            }${card.Goal !== "-" ? `<:hearticon:1128569328929869895> ${card.Goal} ` : ""
                                            }${card.Power !== "-" ? `<:powericon:1128569396261048340>${card.Power} ` : ""
                                            }\n${card.Effect}\n*${card.Fun}*\n${card.Artists.map((a) => {
                                                return `🎨 [${a.Artist}](${a.Link})` + (a.Link.indexOf("https://twitter.com/") === 0 ? ` [(Mirror)](${a.Link.replace("twitter.com", "nitter.net")})` : "")
                                            }).join("\n")}`
                                        ,
